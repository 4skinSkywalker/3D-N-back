function Engine(name) {
    ...
    this.perspective = {
        ...
        MAX: 150,
		snap: 150,
		unit: ($(window).width() > $(window).height()) ? "vh" : "vw",
        change: function (x) {
...
Engine.prototype.populateSiteWrap = function () {
    ...
    s += "<div id=\"grid-3D\" style=\"perspective:" + this.perspective.value + (($(window).width() > $(window).height()) ? "vh" : "vw") + "\">";
    s += "<div class=\"scene\">";
...
Engine.prototype.eventsInit = function () {
	...
	$(window).resize(function	() {
		if (that.perspective.value !== that.perspective.snap) {
			that.perspective.unit = ($(window).width() > $(window).height()) ? "vh" : "vw";
